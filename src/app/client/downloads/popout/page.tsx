...
export default function DownloadsPopoutPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      const { data: profile } = await supabase.from('profiles').select('is_guest, workspace_id').eq('id', user.id).single()
      if (profile) {
        setIsGuest(profile.is_guest)
        if (profile.workspace_id) {
          const { data } = await supabase.from('projects').select('*').eq('workspace_id', profile.workspace_id)
