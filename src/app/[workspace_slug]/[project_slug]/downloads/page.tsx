...
export default function DownloadsPage({ params }: PageProps) {
  const { workspace_slug, project_slug } = React.use(params)
  ...
  // 1. Resolve User and Project IDs
  useEffect(() => {
    async function init() {
      try {
        ...
        console.error('Error resolving workspace parameters:', err)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  // 3. Realtime Subscription to Broadcast Tunnel and download_jobs table
  useEffect(() => {
    if (!userId || !projectId) return
  ...
  }, [userId, projectId, workspace_slug])
  ...
  // 5. Clear completed/failed history
  ...
  // 6. Force download of the generated file
  const handleDownloadFile = async (e: React.MouseEvent, url: string, fileName: string) => {
    ...
    try {
      ...
      // Fallback caso haja restrições de CORS no client side
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-50 dark:bg-neutral-950 p-10">
        <Loader2 className="w-8 h-8 animate-spin text-indigo-600 mb-4" />
        <span className="text-xs font-black uppercase tracking-widest text-neutral-400">
          Carregando Central de Downloads...
        </span>
      </div>
    )
  }
          ...
          {filteredJobs.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-150/50 dark:divide-neutral-800/40">
              {filteredJobs.map((job) => {
                ...
                // Color formatting for file extension badge
                ...
                return (
                  ...
                  >
                    ...
                    {/* Progress bar or State badge (mid) */}
