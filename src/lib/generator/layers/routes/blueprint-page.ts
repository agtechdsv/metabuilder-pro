...
// ─────────────────────────────────────────────────────────────────────────────
// Blueprint Page (Server Component)
// ─────────────────────────────────────────────────────────────────────────────

export function generateBlueprintPage(route: RouteNode): string {
...
export default async function ${mn}BlueprintPage(props: {
      ...
      <div className="px-6 sm:px-10 py-8 flex flex-col sm:flex-row justify-between sm:items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-700">
        <div className="flex items-center gap-5">
          <div className="p-3 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-500/20 text-white shrink-0">
            <DynamicIcon icon="${route.icon || 'Activity'}" size={24} />
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400">
                FLUXOGRAMA DE PROCESSOS &bull; {data.length} {data.length === 1 ? 'ETAPA' : 'ETAPAS'}
              </span>
            </div>
          </div>
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// Blueprint Schema ([route]/schema.ts)
// ─────────────────────────────────────────────────────────────────────────────

export function generateBlueprintSchema(route: RouteNode): string {
  const mn = route.modelName
  const bc = route.blueprintConfig || {
    titleField: 'nome',
    predecessorField: 'predecessor_id',
    scale: 1,
    direction: 'TB',
    animatedEdges: true,
  }
  ...
  const blueprintConfigObj = {
    titleField: bc.titleField || 'nome',
    predecessorField: bc.predecessorField || 'predecessor_id',
    descField: bc.descField || '',
    statusField: bc.statusField || '',
    scale: typeof bc.scale === 'number' ? bc.scale : 1,
    direction: bc.direction || 'TB',
    animatedEdges: bc.animatedEdges !== false,
    // snake_case aliases para compatibilidade total
    title_field: bc.titleField || 'nome',
    predecessor_field: bc.predecessorField || 'predecessor_id',
    desc_field: bc.descField || '',
    status_field: bc.statusField || '',
    animated_edges: bc.animatedEdges !== false,
  }
  ...
  return `// ─────────────────────────────────────────────────────────────────────────────
// Schemas e configurações declarativas para Blueprint de ${route.title}
// ─────────────────────────────────────────────────────────────────────────────
...
export const blueprintConfig = ${blueprintConfigData}
...
// ─────────────────────────────────────────────────────────────────────────────
// Blueprint Client Component ([route]/BlueprintClient.tsx)
// ─────────────────────────────────────────────────────────────────────────────

export function generateBlueprintClient(route: RouteNode): string {
  const mn = route.modelName
  const mnLower = mn.toLowerCase()
  ...
  const modalJsx = isActionOverlay ? (
    isDrawer ? `
      {isModalOpen && (
        ...
        >
          <div 
            className="w-full max-w-md h-full bg-white dark:bg-neutral-900 shadow-2xl border-l border-neutral-200 dark:border-neutral-800 flex flex-col animate-in slide-in-from-right duration-300 relative"
...
${modalFormFieldsHtml}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-neutral-900 rounded-[2rem] border border-neutral-200 dark:border-neutral-800 shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
...
// Carregamento dinâmico sem SSR para segurança do React Flow
const BlueprintBoard = dynamic(
  () => import('@/components/BlueprintBoard').then(mod => mod.BlueprintBoard),
  {
    ssr: false,
    loading: () => (
