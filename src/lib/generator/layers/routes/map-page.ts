...
export function generateMapPage(route: RouteNode): string {
  ...
  // Detecta todas as tabelas relacionadas necessárias para lookups dos filtros e marcadores
...
export default async function ${mn}MapPage(props: {
      ...
      <div className="px-6 sm:px-10 py-8 flex flex-col sm:flex-row justify-between sm:items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-700">
        <div className="flex items-center gap-5">
          <div className="p-3 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-500/20 text-white shrink-0">
            <DynamicIcon icon="${route.icon || 'MapPin'}" size={24} />
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400">
                VISÃO GEOESPACIAL &bull; {data.length} {data.length === 1 ? 'REGISTRO' : 'REGISTROS'}
              </span>
            </div>
          </div>
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// Map Schema ([route]/schema.ts)
// ─────────────────────────────────────────────────────────────────────────────

export function generateMapSchema(route: RouteNode): string {
  const mn = route.modelName
  const mc = route.mapConfig || {
    latField: 'latitude',
    lngField: 'longitude',
    titleField: 'nome',
    zoom: 13,
  ...
  const mapConfigObj = {
    latField: mc.latField || 'latitude',
    lngField: mc.lngField || 'longitude',
    titleField: mc.titleField || 'nome',
    descField: mc.descField || '',
    zoom: typeof mc.zoom === 'number' ? mc.zoom : 13,
    // snake_case aliases para compatibilidade total
    lat_field: mc.latField || 'latitude',
    lng_field: mc.lngField || 'longitude',
    title_field: mc.titleField || 'nome',
    desc_field: mc.descField || '',
  }

  const mapConfigData = JSON.stringify(mapConfigObj, null, 2)
  ...
  return `// ─────────────────────────────────────────────────────────────────────────────
// Schemas e configurações declarativas para Mapa de ${route.title}
// ─────────────────────────────────────────────────────────────────────────────
...
export const mapConfig = ${mapConfigData}
...
// ─────────────────────────────────────────────────────────────────────────────
// Map Client Component ([route]/MapClient.tsx)
// ─────────────────────────────────────────────────────────────────────────────

export function generateMapClient(route: RouteNode): string {
  const mn = route.modelName
  const mnLower = mn.toLowerCase()
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-neutral-900 dark:text-white">
                    {modalMode === 'edit' ? 'Editar Registro' : 'Novo Ponto no Mapa'}
                  </h2>
...
const MapBoard = dynamic(
  () => import('@/components/MapBoard').then(mod => mod.MapBoard),
  {
    ssr: false,
    loading: () => (
