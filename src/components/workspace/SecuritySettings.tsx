...
import { Shield, Smartphone, KeyRound, Lock, AlertCircle, Fingerprint } from 'lucide-react'
...
export default function SecuritySettings({ profile, isOwner }: SecuritySettingsProps) {
  ...
  const [traditionalLogin, setTraditionalLogin] = useState(profile?.traditional_login_enabled !== false)
  const [googleLogin, setGoogleLogin] = useState(profile?.google_login_enabled !== false)
  ...
  useEffect(() => {
    ...
    setTraditionalLogin(profile?.traditional_login_enabled !== false)
    setGoogleLogin(profile?.google_login_enabled !== false)
  ...
  }, [profile?.enforce_mfa, profile?.passkey_enabled, profile?.traditional_login_enabled, profile?.google_login_enabled, profile.id, supabase])
  ...
  const handleSave = async () => {
    // Validação: não pode desligar tudo
    if (!traditionalLogin && !passkeyEnabled && !googleLogin) {
      toast('Você precisa deixar pelo menos um método de login ativo.', 'error')
      return
    }
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          traditional_login_enabled: traditionalLogin,
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[2rem] p-8 shadow-sm">
      ...
      <div className="space-y-6">
        ...
        <div className={`flex items-center justify-between p-6 rounded-2xl border transition-colors ${traditionalLogin ? 'bg-indigo-50/50 border-indigo-100 dark:bg-indigo-500/5 dark:border-indigo-500/20' : 'bg-neutral-50 border-neutral-200 dark:bg-neutral-900/50 dark:border-neutral-800'}`}>
          <div className="flex gap-4 items-start">
            <div className={`p-3 rounded-xl ${traditionalLogin ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400' : 'bg-neutral-200 text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400'}`}>
              <Lock className="w-6 h-6" />
            </div>
            <div>
              <h4 className="text-base font-bold text-neutral-900 dark:text-white">Login Tradicional</h4>
              <p className="text-sm text-neutral-500 mt-1 max-w-md leading-relaxed">
                Permite o acesso através de E-mail e Senha. Este é o método padrão de autenticação.
              </p>
            </div>
          </div>
          <button
            disabled={!isOwner || isSaving}
            onClick={() => setTraditionalLogin(!traditionalLogin)}
            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ${
              traditionalLogin ? 'bg-indigo-600' : 'bg-neutral-300 dark:bg-neutral-700'
            }`}
          >
            <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform shadow-sm ${traditionalLogin ? 'translate-x-7' : 'translate-x-1'}`} />
          </button>
        </div>

        {/* Google Login Toggle */}
        <div className={`flex items-center justify-between p-6 rounded-2xl border transition-colors ${googleLogin ? 'bg-sky-50/50 border-sky-100 dark:bg-sky-500/5 dark:border-sky-500/20' : 'bg-neutral-50 border-neutral-200 dark:bg-neutral-900/50 dark:border-neutral-800'}`}>
          <div className="flex gap-4 items-start">
            <div className={`p-3 rounded-xl flex items-center justify-center ${googleLogin ? 'bg-sky-100 dark:bg-sky-500/20' : 'bg-neutral-200 dark:bg-neutral-800'}`}>
              <svg className={`w-6 h-6 ${googleLogin ? 'opacity-100' : 'opacity-50 grayscale'}`} viewBox="0 0 24 24">
            ...
            <div>
              <h4 className="text-base font-bold text-neutral-900 dark:text-white">Entrar com Google</h4>
              <p className="text-sm text-neutral-500 mt-1 max-w-md leading-relaxed">
                Ofereça acesso rápido em um clique utilizando a conta do Google do usuário, sem necessidade de senhas.
              </p>
            </div>
          </div>
          <button
            ...
            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ${
              googleLogin ? 'bg-sky-500' : 'bg-neutral-300 dark:bg-neutral-700'
            }`}
          >
            <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform shadow-sm ${googleLogin ? 'translate-x-7' : 'translate-x-1'}`} />
          </button>
        </div>
