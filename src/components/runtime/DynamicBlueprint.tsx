...
import { MoreVertical, CheckCircle2, Circle, AlertCircle, PlayCircle, Eye, Pencil, Trash2, Settings2, Wand2, RefreshCcw, Activity, Zap, Search } from 'lucide-react'
...
function DynamicBlueprintContent({
  ...
  const filteredItems = useMemo(() => {
    ...
    return data.filter(row => {
      const title = String(formatFieldValue(getRowVal(row, title_field, titleFieldDef), titleFieldDef, relationalOptions) || '').toLowerCase()
      const desc = desc_field ? String(formatFieldValue(getRowVal(row, desc_field, descFieldDef), descFieldDef, relationalOptions) || '').toLowerCase() : ''
      return title.includes(term) || desc.includes(term)
    })
  }, [data, searchTerm, title_field, titleFieldDef, desc_field, descFieldDef, relationalOptions, getRowVal])

  useEffect(() => {
    if (selectedId) {
      const node = nodes.find(n => n.id === selectedId)
      if (node) {
        setCenter(node.position.x + currentWidth / 2, node.position.y + currentHeight / 2, {
          zoom: 1.2,
          duration: 800,
        })
      }
    }
  }, [selectedId, nodes, setCenter, currentWidth, currentHeight])
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full">
      ...
      <div className="w-full h-[750px] rounded-[2rem] border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-[#0a0a0a] overflow-hidden shadow-sm flex flex-row relative">
        ...
        {/* Painel Lateral de Etapas (Fiel ao Eject) */}
          ...
          <div className="p-3 border-b border-neutral-100 dark:border-neutral-800">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-neutral-100/60 dark:bg-neutral-900 border border-neutral-200/60 dark:border-neutral-800 rounded-xl text-xs outline-none focus:border-indigo-500 transition-all text-neutral-800 dark:text-neutral-200"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-neutral-100 dark:divide-neutral-800/50 custom-scrollbar">
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map(row => {
                ...
                const title = formatFieldValue(getRowVal(row, title_field, titleFieldDef), titleFieldDef, relationalOptions) || 'Sem Título'
                const desc = desc_field ? formatFieldValue(getRowVal(row, desc_field, descFieldDef), descFieldDef, relationalOptions) : ''
