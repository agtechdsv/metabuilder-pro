...
import { useRecordFormRelational } from './hooks/useRecordFormRelational';
import { useRecordFormDetails } from './hooks/useRecordFormDetails';
import { useRecordFormFormulas } from './hooks/useRecordFormFormulas';
...
const { t } = useI18n();
  ...
  const { expandedDetails, setExpandedDetails, loadingSubDetails, setLoadingSubDetails, fetchSubDetailsForRecord } = useRecordFormDetails({
    joins, projectId, tunnelChannel, isTunnelReady, project, secretToken, detailsItemTitles, setFormData: (val: any) => setFormData(val)
  });

  const { formData, setFormData, activeTab, setActiveTab, formRef, handleSubmit } = useRecordFormState({
    initialData, mode, initialTab, logicType, masterModelId, masterModelName, fields, detailTables, onSave, refreshTrigger, setExpandedDetails
  });

  const { relationalOptions } = useRecordFormRelational({
    fields, isTunnelReady, tunnelChannel, project, projectId, secretToken, refreshTrigger
  });

  useRecordFormFormulas({
    fields, formData, setFormData, masterModelName, masterModelId, project
  });
